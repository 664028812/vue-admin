  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <el-form-item prop="iskeepAlive" label="是否启用" width sortable>
...
export default {
  ...
  methods: {
    ...
    load(tree, treeNode, resolve) {
      ...
      Get("getTreeTable", para).then(res => {
    ...
    handleDel() {
      ...
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          //NProgress.start();
          let para = { id: row.id };
          Delete("DeletePermission", para).then(res => {
    ...
    handleAdd() {
      ...
      let para = { pid: 0 };
      Get("getPermissionTree", para).then(res => {
        console.log("getPermissionTree  返回信息            ", res);
        this.options.push(res.response);
      });
    },
    //编辑
    editSubmit: function() {
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$confirm("确认提交吗？", "提示", {}).then(() => {
            ...
            console.log("postPermission  请求数据", para);
            Post("postPermission", para).then(res => {
              console.log("postPermission  响应数据", res);
              if (util.isEmt.format(res)) {
  ...
  mounted() {
    ...
    Get("getInterface", {}).then(res => {
      this.modules = res.response.data;
    });
  }
};
</script>
